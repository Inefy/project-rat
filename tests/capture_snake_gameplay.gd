extends SceneTree
## Capture the production reference snake, including a live attack warning.
...
func capture() -> void:
	...
	for i in range(offsets.size()):
		...
		snake.elite = false
		snake.scale = Vector2.ONE
		snake.global_position = game.player.global_position+offsets[i]
		snake.rotation = facings[i]
		snake.spawn_scale = 1.0
		if i == 1:
			snake.ranged_windup = true
			snake.ranged_clock = .30
			snake.ranged_direction = -offsets[i].normalized()
		if i == 2:
			snake.health = snake.max_health*.72
	...
	root.get_texture().get_image().save_png("res://art/snake/in-game.png")
	game.audio.stop_all()

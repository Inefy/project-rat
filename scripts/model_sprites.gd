...
static func paint(canvas: CanvasItem, kind: String, facing: float, size: float, age: float = 0.0, bounce: float = 0.0, scale_factor: float = 1.0, flash: bool = false) -> void:
	...
	canvas.draw_texture_rect(texture, rect, false, Color(1.7, 1.7, 1.7) if flash else (Color("b9c3ce") if frames.size() == 8 and kind not in ["cat", "fox", "owl"] else Color.WHITE))

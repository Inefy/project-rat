"""Build the reference cat, export GLB, and render production sprite facings.

blender --background --factory-startup --python tools/build_reference_cat.py
blender --background art/cat/reference-cat.blend --python tools/build_reference_cat.py -- --render
"""
import bpy
import json
import math
...
from cat_model import build_cat, IDENTITY
OUT=os.path.join(ROOT,'art','cat')
...
def build():
    ...
    scene=bpy.data.scenes.new('Reference Cat Studio')
    ...
    world=bpy.data.worlds.new('Cat neutral studio')
    ...
    scene.world=world
    parts=build_cat()
    coll=bpy.data.collections.new('Reference Cat - editable parts')
    scene.collection.children.link(coll)
    root=bpy.data.objects.new('cat_root',None);coll.objects.link(root)
    for p in parts:
        for c in list(p.users_collection):c.objects.unlink(p)
        coll.objects.link(p);p.parent=root
    # Product lighting separates black body contours without washing them gray.
    ...
    bpy.ops.object.camera_add(location=(6.6,-11.5,4.4))
    cam=bpy.context.object;cam.name='Reference three-quarter camera';cam.data.type='ORTHO';cam.data.ortho_scale=5.85
    aim(cam,(0,.6,2.43));scene.camera=cam
    ...
    ref=bpy.data.images.load(os.path.join(ROOT,'art','references','cat-design.png'),check_existing=True);ref.pack()
    ...
    scene['reference']='User supplied cat-design.png. Rear anatomy inferred from the single view.'
    ...
    bpy.ops.wm.save_as_mainfile(filepath=os.path.join(OUT,'reference-cat.blend'),compress=True)
    scene.render.filepath=os.path.join(OUT,'cat-preview.png')
    bpy.ops.render.render(write_still=True)
    print('REFERENCE_CAT_BUILD_COMPLETE',flush=True)
...
def export_model(coll,root):
    ...
    bpy.ops.object.join()
    obj=bpy.context.object;obj.name='Cat_Web'
    bpy.ops.object.transform_apply(location=True,rotation=True,scale=True)
    obj.data.calc_loop_triangles()
    triangles=len(obj.data.loop_triangles)
    path=os.path.join(ROOT,'art','models','cat.glb')
    ...
    runtime_path=os.path.join(ROOT,'assets','models','cat.glb')
    ...
    with open(os.path.join(OUT,'cat-stats.json'),'w') as f:json.dump(stats,f,indent=2)
    manifest_path=os.path.join(ROOT,'art','asset-manifest.json')
    with open(manifest_path) as f:manifest=json.load(f)
    manifest['cat']={'directions':8,'collection':'cat','triangles':triangles,'visual_identity':IDENTITY,'source':'art/cat/reference-cat.blend','glb_bytes':stats['glb_bytes'],'materials':3}
    with open(manifest_path,'w') as f:json.dump(manifest,f,indent=2);f.write('\n')
    bpy.data.objects.remove(obj,do_unlink=True)
    print('CAT_STATS '+json.dumps(stats),flush=True)


def render():
    scene=bpy.data.scenes['Reference Cat Studio'];bpy.context.window.scene=scene
    root=bpy.data.objects['cat_root'];cam=scene.camera
    ...
    extent=0
    coll=bpy.data.collections['Reference Cat - editable parts']
    for i in range(8):
    ...
    for i in range(8):
        ...
        scene.render.filepath=os.path.join(ROOT,'assets','sprites',f'cat_{i}.png')
        bpy.ops.render.render(write_still=True)
    print('CAT_SPRITES_COMPLETE',flush=True)

"""Build the reference owl, export GLB, and render production sprite facings.

blender --background --factory-startup --python tools/build_reference_owl.py
blender --background art/owl/reference-owl.blend --python tools/build_reference_owl.py -- --render
"""
import bpy
import json
import math
...
from owl_model import build_owl, IDENTITY
OUT=os.path.join(ROOT,'art','owl')
...
def build():
    ...
    scene=bpy.data.scenes.new('Reference Owl Studio')
    ...
    world=bpy.data.worlds.new('Owl neutral studio')
    ...
    scene.world=world
    parts=build_owl()
    coll=bpy.data.collections.new('Reference Owl - editable parts')
    scene.collection.children.link(coll)
    root=bpy.data.objects.new('owl_root',None);coll.objects.link(root)
    for p in parts:
    ...
    bpy.ops.object.camera_add(location=(0,-15,5.3))
    cam=bpy.context.object;cam.name='Reference three-quarter camera';cam.data.type='ORTHO';cam.data.ortho_scale=7.0
    aim(cam,(0,0,3.05));scene.camera=cam
    ...
    ref=bpy.data.images.load(os.path.join(ROOT,'art','references','owl-design.png'),check_existing=True);ref.pack()
    ...
    scene['reference']='User supplied owl-design.png. Rear anatomy inferred from the single view.'
    ...
    bpy.ops.wm.save_as_mainfile(filepath=os.path.join(OUT,'reference-owl.blend'),compress=True)
    scene.render.filepath=os.path.join(OUT,'owl-preview.png')
    bpy.ops.render.render(write_still=True)
    print('REFERENCE_OWL_BUILD_COMPLETE',flush=True)


def export_model(coll,root):
    # Export one mesh / two material primitives. The source retains named parts.
    bpy.ops.object.select_all(action='DESELECT')
    copies=[]
    ...
    bpy.ops.object.join()
    obj=bpy.context.object;obj.name='Owl_Game'
    bpy.ops.object.transform_apply(location=True,rotation=True,scale=True)
    obj.data.calc_loop_triangles()
    triangles=len(obj.data.loop_triangles)
    path=os.path.join(ROOT,'art','models','owl.glb')
    ...
    runtime_path=os.path.join(ROOT,'assets','models','owl.glb')
    ...
    with open(os.path.join(OUT,'owl-stats.json'),'w') as f:json.dump(stats,f,indent=2)
    manifest_path=os.path.join(ROOT,'art','asset-manifest.json')
    with open(manifest_path) as f:manifest=json.load(f)
    manifest['owl']={'directions':8,'collection':'owl','triangles':triangles,'visual_identity':IDENTITY,'source':'art/owl/reference-owl.blend','glb_bytes':stats['glb_bytes'],'materials':2}
    with open(manifest_path,'w') as f:json.dump(manifest,f,indent=2);f.write('\n')
    bpy.data.objects.remove(obj,do_unlink=True)
    print('OWL_STATS '+json.dumps(stats),flush=True)
...
def render():
    scene=bpy.data.scenes['Reference Owl Studio'];bpy.context.window.scene=scene
    root=bpy.data.objects['owl_root'];cam=scene.camera
    ...
    cam.location=(0,-10,7.3);target=Vector((0,0,3.0));aim(cam,target)
    bpy.context.view_layer.update()
    right=cam.rotation_euler.to_matrix() @ Vector((1,0,0))
    up=cam.rotation_euler.to_matrix() @ Vector((0,1,0))
    extent=0
    coll=bpy.data.collections['Reference Owl - editable parts']
    for i in range(8):
    ...
    for i in range(8):
        ...
        scene.render.filepath=os.path.join(ROOT,'assets','sprites',f'owl_{i}.png')
        bpy.ops.render.render(write_still=True)
        strip_sprite_metadata(scene.render.filepath)
    stats_path=os.path.join(OUT,'owl-stats.json')
    with open(stats_path) as f:stats=json.load(f)
    stats['sprite_bytes']=sum(os.path.getsize(os.path.join(ROOT,'assets','sprites',f'owl_{i}.png')) for i in range(8))
    stats['paint_material']='KHR_materials_unlit'
    with open(stats_path,'w') as f:json.dump(stats,f,indent=2);f.write('\n')
    print('OWL_SPRITES_COMPLETE',flush=True)

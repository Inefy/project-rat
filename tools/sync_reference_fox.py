"""Replace just the fox in the existing cast library from its editable source.

blender --background art/picnic-cast.blend --python tools/sync_reference_fox.py
"""
import bpy
import os
...
gallery_transforms=[]
if gallery:
    old_roots=[o for o in gallery.objects if o.name.startswith('fox_root')]
    for root in old_roots:
        gallery_transforms.append(root.matrix_world.copy())
        for child in list(root.children_recursive):
            bpy.data.objects.remove(child,do_unlink=True)
        bpy.data.objects.remove(root,do_unlink=True)
old=bpy.data.collections.get('fox')
if old:
    for obj in list(old.objects):bpy.data.objects.remove(obj,do_unlink=True)
    bpy.data.collections.remove(old)
path=os.path.join(ROOT,'art','fox','reference-fox.blend')
with bpy.data.libraries.load(path,link=False) as (src,dst):
    dst.collections=['Reference Fox - editable parts']
coll=dst.collections[0];coll.name='fox'
...
print('REFERENCE_FOX_LIBRARY_SYNC_COMPLETE')

"""Build the reference snake, export GLB, and render production sprite facings.

blender --background --factory-startup --python tools/build_reference_snake.py
blender --background art/snake/reference-snake.blend --python tools/build_reference_snake.py -- --render
"""
import bpy
import json
import math
...
from snake_model import build_snake, IDENTITY
OUT=os.path.join(ROOT,'art','snake')
...
def build():
    ...
    scene=bpy.data.scenes.new('Reference Snake Studio')
    ...
    world=bpy.data.worlds.new('Snake neutral studio')
    ...
    scene.world=world
    parts=build_snake()
    coll=bpy.data.collections.new('Reference Snake - editable parts')
    scene.collection.children.link(coll)
    root=bpy.data.objects.new('snake_root',None);coll.objects.link(root)
    for p in parts:
        for c in list(p.users_collection):c.objects.unlink(p)
        coll.objects.link(p);p.parent=root
    # Soft studio lighting keeps the green body and sculpted skin readable.
    ...
    bpy.ops.object.camera_add(location=(13,-2,5.4))
    cam=bpy.context.object;cam.name='Reference three-quarter camera';cam.data.type='ORTHO';cam.data.ortho_scale=6.7
    aim(cam,(0,.35,2.8));scene.camera=cam
    ...
    ref=bpy.data.images.load(os.path.join(ROOT,'art','references','snake-design.png'),check_existing=True);ref.pack()
    ...
    scene['reference']='User supplied snake-design.png. Rear anatomy inferred from the single view.'
    ...
    bpy.ops.wm.save_as_mainfile(filepath=os.path.join(OUT,'reference-snake.blend'),compress=True)
    scene.render.filepath=os.path.join(OUT,'snake-preview.png')
    bpy.ops.render.render(write_still=True)
    print('REFERENCE_SNAKE_BUILD_COMPLETE',flush=True)
...
def export_model(coll,root):
    ...
    bpy.ops.object.join()
    obj=bpy.context.object;obj.name='Snake_Game'
    bpy.ops.object.transform_apply(location=True,rotation=True,scale=True)
    obj.data.calc_loop_triangles()
    triangles=len(obj.data.loop_triangles)
    path=os.path.join(ROOT,'art','models','snake.glb')
    ...
    runtime_path=os.path.join(ROOT,'assets','models','snake.glb')
    ...
    with open(os.path.join(OUT,'snake-stats.json'),'w') as f:json.dump(stats,f,indent=2)
    manifest_path=os.path.join(ROOT,'art','asset-manifest.json')
    with open(manifest_path) as f:manifest=json.load(f)
    manifest['snake']={'directions':8,'collection':'snake','triangles':triangles,'visual_identity':IDENTITY,'source':'art/snake/reference-snake.blend','glb_bytes':stats['glb_bytes'],'materials':3}
    with open(manifest_path,'w') as f:json.dump(manifest,f,indent=2);f.write('\n')
    bpy.data.objects.remove(obj,do_unlink=True)
    print('SNAKE_STATS '+json.dumps(stats),flush=True)
...
def render():
    scene=bpy.data.scenes['Reference Snake Studio'];bpy.context.window.scene=scene
    root=bpy.data.objects['snake_root'];cam=scene.camera
    ...
    cam.location=(0,-10,7.3);target=Vector((0,.35,2.75));aim(cam,target)
    bpy.context.view_layer.update()
    right=cam.rotation_euler.to_matrix() @ Vector((1,0,0))
    up=cam.rotation_euler.to_matrix() @ Vector((0,1,0))
    extent=0
    coll=bpy.data.collections['Reference Snake - editable parts']
    for i in range(8):
    ...
    for i in range(8):
        ...
        scene.render.filepath=os.path.join(ROOT,'assets','sprites',f'snake_{i}.png')
        bpy.ops.render.render(write_still=True)
        strip_sprite_metadata(scene.render.filepath)
    stats_path=os.path.join(OUT,'snake-stats.json')
    with open(stats_path) as f:stats=json.load(f)
    stats['sprite_bytes']=sum(os.path.getsize(os.path.join(ROOT,'assets','sprites',f'snake_{i}.png')) for i in range(8))
    stats['paint_material']='KHR_materials_unlit'
    with open(stats_path,'w') as f:json.dump(stats,f,indent=2);f.write('\n')
    print('SNAKE_SPRITES_COMPLETE',flush=True)
